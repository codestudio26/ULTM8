 ...
 * service asserts the CALLER is Staff whenever `overrideReason` is present, or
 * whenever `studentId` names someone other than the caller (a Staff member booking
 * on a Student's behalf). Both are omitted for the ordinary Student self-service
...
export class BookClassDto {
  @ApiPropertyOptional({ description: 'Staff-only: book on behalf of this Student instead of the caller.' })
  @IsOptional()
  @IsUUID()
  studentId?: string;
