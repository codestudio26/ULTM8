 ...
 * RoleGrantsService's target-user existence/verification check, which hits the same
 * structural gap (a School Owner/Manager inviting a brand-new Instructor/Branch Staff
 * has no RoleGrant linking them to that person yet, so `user_self_or_shared_school`
 * would correctly show nothing). Both call sites select the minimum fields needed
 * (never a full profile) and never write anything — this connection deliberately
 * cannot. Its RLS policy (user_auth_lookup) has no tenant restriction by design; don't
 * broaden a call site here into a general-purpose cross-tenant User read.
 */
@Injectable()
