 ...
 * existed before despite the RLS policies already anticipating them. No
 * Student-name lookup endpoint exists yet (same gap InstructorFormModal's own
 * comment already flags for the identical problem), so rows show a truncated
 * Student id. No write actions here yet (cancel/override-reason/withdraw
 * already exist as Student/self-service or override endpoints — this phase is
 * visibility only, matching TransactionsPage's own "read-only first" scoping). */
        ...
        <Card>
          ...
          {bookings.length === 0 ? (
            ...
            <Table<BookingResponse>
              rows={bookings}
              columns={[
                { key: 'student', header: 'Student', render: (b) => b.studentId.slice(0, 8) },
        ...
        {waitlist.length === 0 ? (
          ...
          <Table<WaitlistEntryResponse>
            ...
            columns={[
              ...
              { key: 'student', header: 'Student', render: (e) => e.studentId.slice(0, 8) },
