...
- **[CONFIRMED]** A policy that needs to check for the *existence* of a row belonging to someone other than the calling session — "does any RoleGrant exist for this School," not "does the caller's own RoleGrant exist" — must not do that as a plain inline subquery. An inline subquery against an RLS-protected table is itself filtered by that table's own policies, from the calling session's restricted view, not from an unfiltered view of the whole table — so a negated check ("not exists") silently becomes "not exists that I'm already allowed to see," which inverts the security property instead of enforcing it. Confirmed the hard way in Phase 2 (TenantsModule): this exact mistake shipped twice — once inside `RoleGrant`'s own policy (self-referential `EXISTS`, causing Postgres error 42P17 infinite recursion), then again inside `School`'s policy (a `NOT EXISTS` into `RoleGrant`, which silently let any tenant read and write any other tenant's School until CI caught it). The fix both times: wrap the check in its own `SECURITY DEFINER STABLE` SQL function, owned by a dedicated `ultm8_rls_helper` role (`NOLOGIN`, `BYPASSRLS`) — its internal query reads the table unfiltered, so "exists"/"not exists" means what it says. A check against the *caller's own* rows doesn't need this (`rolegrant_self_only` already guarantees a caller sees their own rows regardless of any other policy) — only checks that need visibility into rows belonging to someone else do. (`apps/api/prisma/migrations/20260904000000_fix_rolegrant_rls_recursion`, `20260906000000_fix_school_notexists_rls_blindspot`)
- **[UNRESOLVED]** `user_self_or_shared_school` (Phase 1, `20260902000000_init`) has the same structural weakness on its "theirs" side (a positive `EXISTS` into another user's `RoleGrant` rows) — but fails *closed*, not open: two Instructors at the same School currently can't see each other's `User` profile through this clause, a functional gap rather than a cross-tenant exposure. Found during the Phase 2 audit above, deliberately not fixed as part of it — flagged for its own prioritization whenever staff-visibility work is scoped, not urgent enough to have held up Phase 2.
