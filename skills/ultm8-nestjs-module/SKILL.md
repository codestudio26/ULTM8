...
- **[CONFIRMED]** As of Phase 3, `apps/school-portal` is real: Login/Register/Verify-OTP/Forgot-Reset-passcode, self-service School onboarding (Decision 79), Branch CRUD, and a Staff invite/revoke flow scoped to Decision 80/81. `packages/api-client` is generated (not a placeholder) via `openapi-typescript` against `apps/api`'s live OpenAPI spec.

## 7. Unresolved — do not build against a guess

- **[CONFIRMED]** JWT claims are only ever rebuilt at login (Phase 1 design, `apps/api/src/auth`) — a `RoleGrant` change (new or revoked) has no effect on an already-issued access token until it expires (≤15 minutes) or the holder logs in again. This is an accepted tradeoff, not a bug: the short TTL itself is what bounds a revoked grant's blast radius. Applies uniformly regardless of who changed the grant — the token holder's own action (e.g. self-service School creation) or someone else's (e.g. a School Owner inviting them as Instructor/Branch Staff, Decision 80/81).
- **[CONFIRMED]** A narrow, scoped exception to the above is approved and in progress: `SchoolsService.create()` will re-mint the caller's access token (a new `AuthService.issueAccessToken(userId)` method, `AuthModule` already exports `AuthService` so this needs no new cross-module wiring) and return it in the response, so a self-service School's own owner sees their new `SCHOOL_OWNER_MANAGER` grant immediately rather than needing to log out and back in (the bug Phase 3's `CreateSchoolPage` had to work around). Scoped deliberately narrow: this fixes only the case where the caller changes their *own* grants. It does **not** and cannot cover the RoleGrant-invite case (School Owner grants someone *else* a role) — the invitee isn't present in that HTTP exchange at all, so there's no response to hand them a fresh token through. That case stays inside the accepted staleness window above.
- **[UNRESOLVED]** A general refresh-token endpoint (Spec §8.3 describes one; `AuthModule`'s `login()` returns only `{ accessToken }`, nothing implements it) would close the remaining staleness window — including the invite case the narrow fix above can't reach — and remove the 15-minute forced-relogin generally. Deliberately **not** being built now: it's a real security-surface addition (a token-storage table, rotation, reuse detection, and a client-side delivery mechanism — an httpOnly cookie would need CORS/credentials configuration `apps/api` doesn't have yet) improving on an already-accepted tradeoff, not fixing something broken. Worth its own scoping pass whenever it becomes a real priority, not something to fold into an unrelated fix. Tracked separately from the `user_self_or_shared_school` RLS gap in `ultm8-tenant-isolation` §3 (different subsystem).
